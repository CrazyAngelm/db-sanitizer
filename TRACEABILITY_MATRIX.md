...
> **Изменение по указанию заказчика:** исходный локальный Ollama/qwen happy path
> заменён на OpenRouter `deepseek/deepseek-v4-flash`. Поэтому исходные literal
> критерии AC-001/AC-010 и Ollama-часть AC-071 являются согласованным
> отклонением, а не заявляются как пройденные.
...
| FR-03 | Entity-specific constraints и OpenRouter-генерация правдоподобных synthetic строк | AC-012, AC-013 |
| FR-04 | LangGraph generation agent вызывает OpenRouter (согласованное отклонение от Ollama) | AC-011; AC-010 waived |
...
| IR-01 | PostgreSQL, Greenmask, LangGraph, OpenRouter, psycopg, Pydantic, pytest | AC-071 partial / Ollama waived |
...
| NFR-03 | Server-side cursor, Greenmask streaming, batch LLM | AC-050..053 |
